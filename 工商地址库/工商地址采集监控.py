...
class test():
    def __init__(self):
        self.r = redis.Redis(host='192.168.0.230', port=6379, db=14, decode_responses=True)
        self.ago_data_count = self.r.scard('company_info')

    def count(self, start_time):
        data_count = self.r.scard('company_info')
